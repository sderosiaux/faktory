...
func (m *Memory) Add(ctx context.Context, messages []Message, userID string, opts ...Option) (*AddResult, error) {
	...
	if count, err := m.store.CountFacts(userID, o.namespace); err == nil {
		...
		// Auto-consolidate: summarize session when fact count exceeds threshold
		if m.cfg.ConsolidateThreshold > 0 && count > m.cfg.ConsolidateThreshold {
			go func() {
				if err := m.Summarize(context.Background(), messages, userID, opts...); err != nil {
					m.log.Warn("auto-consolidate failed", "err", err, "total_facts", count, "threshold", m.cfg.ConsolidateThreshold)
				}
			}()
		}
	}
...
// Prune removes facts matching the given criteria. Use DryRun to preview.
func (m *Memory) Prune(ctx context.Context, userID string, opts PruneOptions, callOpts ...Option) (*PruneResult, error) {
	if userID == "" {
		return nil, fmt.Errorf("user_id is required")
	}
	o := resolveOpts(callOpts)

	facts, err := m.store.PruneFacts(userID, o.namespace, opts)
	if err != nil {
		return nil, fmt.Errorf("prune facts: %w", err)
	}

	return &PruneResult{Pruned: facts, Count: len(facts)}, nil
}
